    ...
    return embeds.length > 1
      ? new Pagination()
          ...
          .setPaginationCollector({
            ...
            secondaryUserInteraction: true,
          })
          .setFooter({ enable: true })
          .send()
      : interaction.editReply(embeds.length ? { embeds: embeds } : message);
  else return embeds.length ? { embeds: embeds.slice(0, 1) } : message;
}

async function formatError(status, statusText, interaction, message) {
  if (status)
    message = `An error occurred handling your request: ${status} ${statusText}`;
  else message = `An error occurred handling your request: ${message}`;
